...
export default async function ProfilePage() {
  ...
  const [profileRes, skillsRes, sectorsRes, profSkillsRes, profSectorsRes] = await Promise.all([
    supabase.from("profiles").select("role, status, first_name, surname, linkedin_url, grad_year, bio, working_on").eq("id", user.id).single(),
  ...
  return (
    <div className="min-h-screen bg-bg-primary flex flex-col">
      <AppNav active="profile" isApproved={profile.status === "approved"} />
