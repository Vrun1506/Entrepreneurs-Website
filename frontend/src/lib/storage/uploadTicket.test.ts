...
const { issueTicket, verifyTicket, uploadsEnabled } = await import("./uploadTicket");
...
describe("upload tickets", () => {
  it("round-trips the claims it was issued with", () => {
    const claims = verifyTicket(issueTicket({ userId: USER, key: KEY }));
  ...
  it("rejects a ticket whose payload was edited", () => {
    ...
    const [header, payload, signature] = issueTicket({ userId: USER, key: KEY }).split(".");
  ...
  it("rejects a ticket signed with a different secret", () => {
    const token = issueTicket({ userId: USER, key: KEY });
